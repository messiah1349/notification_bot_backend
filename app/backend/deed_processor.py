...
from sqlalchemy.orm import query, decl_api
...
class TableProcessor:
    ...
    async def _get_filtered_data(self, table_model, filter_values: dict) -> list['table_model']:
        ...
        for filter_column in filter_values:
            query = query.filter(
                getattr(table_model, filter_column) == filter_values[filter_column])
...
class DeedProcessor(TableProcessor):
    ...
    async def get_all_active_deeds(self) -> list[Deed]:
        ...
        try:
            ...
            logger.info("all active deeds was passed")
    ...
    async def add_notification(self, deed_id: int, notification_time: datetime) -> Response(int, Deed | str):
        ...
        try:
            async with self.async_sessionmaker() as session:
                ...
                await session.refresh(deed)
                logger.info(
                    f"notification for {deed_id=} was set to {notification_time}")
                return Response(0, deed)
        except Exception as e:
            logger.error(
                f"notification for {deed_id=} was NOT set to {notification_time}, exception - {e}")
    ...
    async def get_deed_for_user(self, telegram_id: int) -> Response(int, list[Deed]):
        ...
        try:
            ...
            logger.error(
                f"Cannot return deeds for user {telegram_id=} error = {e}")
            return Response(1, e)
    ...
    async def rename_deed(self, deed_id: int, new_deed_name: str):
        ...
        try:
            ...
            logger.error(
                f"{deed_id=} was NOT renamed to {new_deed_name}, exception - {e}")
            return Response(1, e)
